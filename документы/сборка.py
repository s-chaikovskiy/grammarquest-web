...
def _оглавление(d: Document, пункты, страницы=None, заголовок: str = "Мазмұны"):
    """
    ...
    """
    з = d.add_paragraph()
    # Русские документы до 13.09.2026 открывались казахским «Мазмұны»:
    # заголовок был зашит сюда. Теперь его задаёт модуль текста.
    r = з.add_run(заголовок)
...
# Ширина рисунка по умолчанию и потолок высоты. Потолок появился после того,
# как снимок «профиля» пришёл размером 800×5251: при ширине 7,2 см это 47 см,
# вдвое выше листа. Word такую картинку не уменьшает — он её обрезает,
# а заголовок перед ней остаётся один на пустой странице.
ШИРИНА_РИСУНКА = 7.2      # см
ПОТОЛОК_РИСУНКА = 16.0    # см — больше половины листа рисунок занимать не должен
МИНИМУМ_РИСУНКА = 4.0     # см — уже этого снимок экрана не разобрать


def _картинка(d, путь, подпись):
    from PIL import Image

    ш_точек, в_точек = Image.open(путь).size
    ширина = ШИРИНА_РИСУНКА
    высота = ширина * в_точек / ш_точек
    if высота > ПОТОЛОК_РИСУНКА:
        # Вписываем по высоте, а не по ширине.
        ширина = ПОТОЛОК_РИСУНКА * ш_точек / в_точек
        высота = ПОТОЛОК_РИСУНКА
    if ширина < МИНИМУМ_РИСУНКА:
        raise SystemExit(
            f"{путь.name}: {ш_точек}×{в_точек} — слишком вытянутый снимок. "
            f"Чтобы влезть по высоте, его пришлось бы сузить до {ширина:.1f} см, "
            f"а там ничего не разглядеть. Снимайте кадр в размер экрана "
            f"(документы/скриншоты-экранов.mjs) и подрезайте пустой низ "
            f"(документы/подрезать-скриншоты.py).")

    p = d.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p.add_run().add_picture(str(путь), width=Cm(ширина))
    # Рисунок и его подпись не должны разъезжаться по разным листам.
...
def собрать(блоки, d: Document, база: pathlib.Path):
    ...
    for блок in блоки:
        ...
        if вид in ("h1", "h2", "h3"):
            ...
            # Заголовок держим при том, что за ним идёт: иначе он остаётся
            # один на листе, а раздел начинается со следующего. Так и вышло
            # с «7. Прогресті бақылау» — заголовок на 16-й странице, а всё
            # остальное на 17-й.
            з.paragraph_format.keep_with_next = True
            if вид == "h1":
                _линейка(з)
        elif вид == "p":
...
def страницы_разделов(pdf: pathlib.Path, разделы) -> list[int]:
    ...
    листы = [плоско(лист["текст"]) for лист in json.loads(вывод)]
    номера = []
...
def построить(модуль, скрины: pathlib.Path, страницы=None) -> pathlib.Path:
    ...
    _оглавление(d, разделы, страницы, getattr(модуль, "СОДЕРЖАНИЕ", "Мазмұны"))
...
def main():
    import руководство
    import руководство_ru
    import техническое
    import техническое_ru

    собрать_чтец()
    скрины = HERE / "скриншоты"
    # Четыре документа: два казахских для подачи и два русских для ученика.
    for модуль in (техническое, руководство, техническое_ru, руководство_ru):
        разделы = [b[1] for b in модуль.БЛОКИ if b[0] == "h1"]
    ...
    # при фактических 210.
    # Сначала скелет: если языки разъехались, остальные проверки будут
    # мерить уже разные документы.
    #
    # Проверки идут все подряд, даже если одна упала, — чтобы увидеть все
    # расхождения за один прогон. Но сборка обязана упасть после них:
    # 13.09 сверка нашла 14 следов старого имени, а сборка вернула код 0,
    # и по коду это выглядело как успех.
    упавшие = []
    проверки = [
        "сверка-структуры.py",
        "сверка-чисел.py",
        # И вёрстку: пустая страница появляется от рисунка, который не влез,
        # и по числам её не видно.
        "проверка-вёрстки.py",
        # И полноту: приложение растёт быстрее текстов о нём.
        "сверка-возможностей.py",
    ]
    for проверка in проверки:
        print()
        if subprocess.run([sys.executable, str(HERE / проверка)], check=False).returncode:
            упавшие.append(проверка)
    if упавшие:
        raise SystemExit(f"\nСборка не прошла проверки: {', '.join(упавшие)}")
