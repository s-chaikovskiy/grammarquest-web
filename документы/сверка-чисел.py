...
def факты() -> dict[str, int]:
    ...
    return {
        ...
        # Сколько пунктов в разделе «Справка». В руководстве стояло «три»,
        # когда их стало четыре: пункт «Как устроено приложение» добавили
        # в приложение, а в текст — нет.
        "бөліктен": len(re.findall(r"^\s*to: '/", (ROOT / "src" / "screens"
                                                   / "ReferenceHubScreen.tsx")
...
# Некоторые числа в казахском тексте написаны словом: «үш бөліктен тұрады».
# Цифру там ставить неестественно, а сверять надо — именно это место
# и разъехалось, когда разделов «Справки» стало четыре.
СЛОВОМ = {"бір": 1, "екі": 2, "үш": 3, "төрт": 4, "бес": 5,
          "алты": 6, "жеті": 7, "сегіз": 8, "тоғыз": 9, "он": 10,
          "один": 1, "два": 2, "три": 3, "четыре": 4, "четырёх": 4,
          "пять": 5, "шесть": 6, "семь": 7, "восемь": 8, "девять": 9, "десять": 10}


def в_число(найдено: str) -> int:
    return int(найдено) if найдено.isdigit() else СЛОВОМ[найдено]
...
ШАБЛОНЫ = {
    ...
    "бөліктен": r"(бір|екі|үш|төрт|бес)\s+бөліктен",
    # Те же утверждения в русских версиях документов. Без них проверка
    # пробегала по четырём файлам, а на деле смотрела только два.
    "сабақ_ru": r"(\d+)\s+урок",
    "тапсырма_ru": r"(\d+)\s+задани",
    "жаттығу түрі_ru": r"(семь|\d+)\s+типов заданий",
    "сөз_ru": r"(\d+)\s+слов",
    # Граница слова обязательна: без неё «193 правильных ответах»
    # читалось как «193 правила».
    "ереже_ru": r"(\d+)\s+правил(?:а|о)?\b",
    "тақырып_ru": r"(\d+)\s+тем[аыу]?\b",
    "жазба_ru": r"(\d+)\s+запис",
    "автотест_ru": r"(\d+)\s+автотест",
    "тексеру_ru": r"(\d+)\s+проверок контраста",
    "КБ_ru": r"(\d+)\s*КБ",
    "бөліктен_ru": r"из (одной|двух|трёх|четырёх|пяти) частей",
    "КБ": r"(\d+)\s*КБ",
}
...
def main() -> None:
    ф = факты()
    # Русские ключи означают ровно то же, что казахские: «26 уроков» и
    # «26 сабақ» — одно утверждение на двух языках.
    for ключ in list(ф):
        ф[f"{ключ}_ru"] = ф[ключ]
    ...
    for файл in sorted(HERE.glob("*.docx")):
        ...
        for ключ, шаблон in ШАБЛОНЫ.items():
            for найдено in {в_число(m) for m in re.findall(шаблон, t)}:
