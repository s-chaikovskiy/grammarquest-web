# -*- coding: utf-8 -*-
"""
Grammar Dialogue Quest — защита научного проекта. Сборка колоды.

Восемь слайдов, по порядку доклада: кто это сделал и как научился · зачем
это нужно · что создано · какими сервисами · что здесь особенного ·
попробуйте сами · что дальше и какая нужна поддержка.

Технических подробностей намеренно мало. Задача доклада — чтобы приложением
заинтересовались и помогли развивать его, а не чтобы жюри разобралось
в устройстве кода. Прежние версии колоды — в архив/.
...
def build(lang: str):
    ...
    # ═════════════════════════════════════════════════ 02 · как это появилось
    s, y = sl(T["s02_eyebrow"])
    y = title(s, T["s02_title"], y, max_lines=2, start=36, minimum=23)
    ...
    y = title(s, T["s03_title"], y, max_lines=2, start=38, minimum=24)
    ...
    n = len(T["s04_sections"])
    gap = 0.24
    cw = (CW - gap * (n - 1)) / n
    bottom = yy
    for i, (head, body) in enumerate(T["s04_sections"]):
        xx = ML + i * (cw + gap)
    ...
    y = title(s, T["s05_title"], y, dark=True, max_lines=1, start=40)
    y = lede(s, T["s05_lede"], y + 0.16, dark=True, w=CW * 0.86)
    yy = y + 0.50
    lx, lw = col(0, 7)
    rx, rw = col(7, 5)
    b = block(s, [T["s05_left_tag"]], lx + 0.30, yy + 0.30, lw - 0.60, MONO_SB, 8.5,
              SIGNAL_L, leading=11, tracking=0.16, caps=True)
    b = bullets(s, T["s05_left"], lx + 0.30, b + 0.20, lw - 0.60, dark=True, size=11.5, gap=0.16)
    panel_behind(s, lx, yy, lw, bottom=b, pad=0.30, dark=True, accent=SIGNAL)
    b2 = block(s, [T["s05_right_tag"]], rx + 0.30, yy + 0.30, rw - 0.60, MONO_SB, 8.5,
    ...
    y = title(s, T["s06_title"], y, dark=True, max_lines=1, start=42)
    y = lede(s, T["s06_lede"], y + 0.18, dark=True, w=CW * 0.82)
    yy = y + 0.52
    cards = T["s06_cards"]
    gap = 0.22
    cw = (CW - gap * 3) / 4
